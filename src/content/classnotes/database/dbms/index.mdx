...
Habilitar root para acesso externo:
...
```bash
$ docker exec -it mysql mysql -u root -p \
  -e "ALTER USER 'root'@'%' IDENTIFIED WITH mysql_native_password BY 'secret'; FLUSH PRIVILEGES;"
Enter password:
```
...
mysql> INSERT INTO hosts (name, address) VALUES ('Google DNS', '8.8.8.8');
...
| Role        | Descrição                                                      |
| ----------- | -------------------------------------------------------------- |
| `read`      | Permissão apenas leitura em um banco de dados                  |
| `readWrite` | Permissão de leitura e escrita em um banco de dados            |
| `dbAdmin`   | Permissões administrativas do banco (criar índices, etc)       |
| `userAdmin` | Permissão para gerenciar usuários do banco                     |
| `dbOwner`   | Combinação de `readWrite`, `dbAdmin` e `userAdmin`             |
| `root`      | Acesso total a todos os bancos (super usuário)                 |
...
```
    ...
    <tr>
      <td><strong>Deletar Usuário</strong></td>
      <td>
...
```
    ...
    <tr>
      <td><strong>Grant (Banco)</strong></td>
      <td>
...
```
    ...
    <tr>
      <td><strong>Grant (Tabela)</strong></td>
      <td>
...
```
    ...
    <tr>
      <td><strong>Revoke (Banco)</strong></td>
      <td>
...
```
    ...
    <tr>
      <td><strong>Revoke (Tabela)</strong></td>
      <td>
...
```
    ...
    <tr>
      <td><strong>Aplicar Permissões</strong></td>
      <td>
...
```
    ...
    <tr>
      <td><strong>Listar Usuários</strong></td>
      <td>
