// subject data
export const SUBJECT = {
  CLASSROOM_URLS: {
    A: 'https://classroom.google.com/c/NjYzNjMwNzIzNzg1',
  },
  CHAT_URL: 'https://mail.google.com/mail/u/0/#chat/space/AAAAAji4jww',
  PROJECTS_URL: 'https://padlet.com/lucachaves/projetodw20241',
  GITHUB_URLS: {
    A: 'https://github.com/luizchaves/dw-2024-1',
  },
  SCHEDULES: [
    {
      room: 'Lab 58',
      day: 'Segunda',
      start: '07:00',
      end: '08:40',
    },
    {
      room: 'Lab 58',
      day: 'Quarta',
      start: '07:00',
      end: '10:40',
    },
  ],
};

// project data
export const GITHUB_EDIT_URL = 'https://github.com/ifpb/dw/tree/main';

export const CONTENT_SLUGS = [
  'markup/languages',

  'html/introduction',

  'css/introduction',
  'package/bootstrap',
  'css/form',

  'ecma/introduction',
  'ecma/variable',
  'ecma/expression-and-operator',
  'ecma/statements',
  'ecma/function',
  'ecma/modules',
  'ecma/array',
  'ecma/string',
  'ecma/object',

  'w3c/browser-objects',
  'w3c/dynamic-elements',
  'package/vite',
  'w3c/event-handling',
  'w3c/dom-api',
  'w3c/local-storage',
  'package/json-server',
  'w3c/fetch-api',
  // 'package/axios',
  'package/chartjs',

  'nodejs/http',
  'expressjs/introduction',
  'expressjs/system-call',
  'expressjs/api',
  'expressjs/mvc',
  'database/dbms',
  'database/sql',
  'expressjs/db-simple',
  'expressjs/prismajs-simple',
  'expressjs/prismajs-relation',
  'expressjs/prismajs-user',
  'expressjs/auth',
  // docker
  // test
  // ts
  // documentation

  'extra/next-steps',
  'extra/exercises',
];

export const MENU = [
  {
    title: 'Disciplina',
    url: '/dw/',
  },
  {
    title: 'Conteúdos',
    url: '/dw/classnotes/',
  },
];
